...
export class SSFirework {
    ...
    setVelocity(i, speed) {
        ...
        if (this.explosionPattern === "circular") {
            ...
            this.velocities[i * 3 + 2] = (Math.random() - 0.5) * speed;
        } else if (this.explosionPattern === "spiral") {
            const spiralAngle = (i / this.particleCount) * 2 * Math.PI;
            const radius = 0.5;
            this.velocities[i * 3] = radius * Math.cos(spiralAngle) * speed;
            this.velocities[i * 3 + 1] = radius * Math.sin(spiralAngle) * speed;
            this.velocities[i * 3 + 2] = (Math.random() - 0.5) * speed;
        }
    }
